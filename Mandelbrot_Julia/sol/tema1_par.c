...
// initializam global variabile
params par1, par2;
int width1, height1, width2, height2;
int **result1, **result2;


// functie de calcul minim
...
{
	int w, h, i;
	int start1 = thread_id * (double)width/ P;
    int end1 = minim((thread_id + 1) * (double)width / P, width);

    pthread_barrier_wait(&barrier);

	// paralelizam algorimtul de Julia cu start si end
	...
	int h1 = height / 2;
	int start2 = thread_id * (double)h1/ P;
    int end2 = minim((thread_id + 1) * (double)h1 / P, h1);

	// transforma rezultatul din coordonate matematice in coordonate ecran
	// paralelizam si acest for
...
void *f(void *arg){
	...
	// apelam functie de Julia
	run_julia(&par1, result1, width1, height1, thread_id);
	// bariera
    pthread_barrier_wait(&barrier);
	// scriem in fisier
	...
	// apelam functia de Mandelbrot
	run_mandelbrot(&par2, result2, width2, height2, thread_id);
	// bariera
    pthread_barrier_wait(&barrier);
	// scriem in fisier
